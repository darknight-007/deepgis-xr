...
const rasterLayers = {
    'bf_aug_2020_raster': {
        ...
        center: [33.78210534131368, -111.26527270115186, 23]
    },
    'bf_oct_2020_raster': {
...
function initRasterLayers(map) {
    ...
    Object.entries(rasterLayers).forEach(([id, config]) => {
        ...
        // Add event listener for when layer is added to map
        layer.on('add', () => {
            // Set view to layer's center point with zoom level 23
            map.setView([config.center[0], config.center[1]], 23);
        });
    ...
    map.setView([firstLayer.center[0], firstLayer.center[1]], 23);
